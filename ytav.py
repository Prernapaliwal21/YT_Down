        ...
        if self.radio_val.get():
            ...
            try:
                a[self.stream_button.get()]\
                    .download(quiet=True,
                              filepath=self.folder_path)
                # sound = AudioSegment.from_file(self.folder_path+"Ang Laga De Re - Afrosamuraiist")
                # sound.export(self.folder_path+"Ang Laga De Re - Afrosamuraiist", format="mp3", bitrate="128k")
                #
            except FileExistsError:
